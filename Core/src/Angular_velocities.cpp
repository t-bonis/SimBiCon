...
void Angular_velocities::update(const Subject_interface* subject)
{
	...
	{
		for (size_t i = 0; i < gait_analyzer->angular_velocity.size(); i++)
		{
			auto chart1 = &m_two_lines_charts[i * 3];
			auto chart2 = &m_two_lines_charts[i * 3 + 1];
			auto chart3 = &m_two_lines_charts[i * 3 + 2];

			for (size_t j = 0; j < gait_analyzer->angular_velocity[i].size(); j = j + SimGlobals::m_interval_between_measure)
			{
				const auto y11 = gait_analyzer->angular_velocity[i][j].x;
				const auto y12 = gait_analyzer->angular_velocity[i][j].y;
				const auto y13 = gait_analyzer->angular_velocity[i][j].z;

				const auto y21 = gait_analyzer->angular_velocity_reference[i][j].x;
				const auto y22 = gait_analyzer->angular_velocity_reference[i][j].y;
				const auto y23 = gait_analyzer->angular_velocity_reference[i][j].z;
