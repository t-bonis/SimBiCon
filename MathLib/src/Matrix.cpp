...
*/
void Matrix::set_to_outer_product(const Vector3d& a, const Vector3d& b)
{
	resizeTo(3, 3);
...
void Matrix::resizeTo(int m, int n)
{
	...
	if (this->matrix->owner == 1 && this->matrix->block != nullptr && this->matrix->block->size >= size_t(m) * n)
	{
		this->matrix->size1 = m;
		this->matrix->size2 = n;
		this->matrix->tda = n;
...
*/
void Matrix::shallowCopy(const Matrix& other, int startRow, int startCol, size_t endRow, size_t endCol)
{
	//we have to first delete the current matrix
	gsl_matrix_free(this->matrix);
	//and create the shallow copy
	if (endRow == size_t(-1)) endRow = other.matrix->size1;
	if (endCol == size_t(-1)) endRow = other.matrix->size2;
...
Vector3d Matrix::operator *(const Vector3d& other)
{
	...
	if (this->matrix->size1 != 3 || this->matrix->size2 != 3)
		throw std::logic_error("Can only use the * operator between a 3x3 matrix and a vector3d object!");
	result.x = MATRIX_AT(this->matrix, 0, 0) * other.x + MATRIX_AT(this->matrix, 0, 1) * other.y + MATRIX_AT(this->
		matrix, 0, 2) * other.z;
	result.y = MATRIX_AT(this->matrix, 1, 0) * other.x + MATRIX_AT(this->matrix, 1, 1) * other.y + MATRIX_AT(this->
		matrix, 1, 2) * other.z;
	result.z = MATRIX_AT(this->matrix, 2, 0) * other.x + MATRIX_AT(this->matrix, 2, 1) * other.y + MATRIX_AT(this->
		matrix, 2, 2) * other.z;
...
void Matrix::postMultiplyVector(const Vector3d& v, Vector3d& result)
{
	if (this->matrix->size1 != 3 || this->matrix->size2 != 3)
		throw std::logic_error("Can only use the * operator between a 3x3 matrix and a vector3d object!");
	result.x = MATRIX_AT(this->matrix, 0, 0) * v.x + MATRIX_AT(this->matrix, 0, 1) * v.y + MATRIX_AT(this->matrix, 0, 2)
		* v.z;
	result.y = MATRIX_AT(this->matrix, 1, 0) * v.x + MATRIX_AT(this->matrix, 1, 1) * v.y + MATRIX_AT(this->matrix, 1, 2)
		* v.z;
	result.z = MATRIX_AT(this->matrix, 2, 0) * v.x + MATRIX_AT(this->matrix, 2, 1) * v.y + MATRIX_AT(this->matrix, 2, 2)
		* v.z;
}
...
void Matrix::setToInverseOf(const Matrix& a, double t)
{
	if (a.matrix->size1 != a.matrix->size2)
		throw std::logic_error("Cannot invert a matrix that is not square");
	...
	for (int i = 0; i != DIM; i++)
	{
		double val = MATRIX_AT(tmp.matrix, i, i); /* find pivot */
		int ind = i;
		for (j = i + 1; j != DIM; j++)
		{
		...
		if (ind != i)
		{
			for (j = 0; j != DIM; j++)
			{
				double val2 = MATRIX_AT(this->matrix, i, j);
