...
TransformationMatrix::TransformationMatrix(const Matrix& other) : Matrix(other)
{
	...
	{
		this->resizeTo(4, 4);
		throw std::logic_error("CONSTRUCTOR: Matrix passed in did not have the correct dimensions!");
	}
}
...
TransformationMatrix& TransformationMatrix::operator =(const Matrix& other)
{
	...
	if (other.getColumnCount() != 4 || other.getRowCount() != 4)
		throw std::logic_error("COPY OPERATOR: Matrix passed in did not have the correct dimensions!");
...
void TransformationMatrix::setToProductOf(const Matrix& a, const Matrix& b, bool transA, bool transB)
{
	...
	{
		this->resizeTo(4, 4);
		throw std::logic_error("Matrix Product: Matrices passed in did not have the correct dimensions!");
	}
}
...
/**
	...
	If the desired product does not result in a 4x4 matrix an error is thrown.
*/
void TransformationMatrix::setToProductOf(const TransformationMatrix& a, const TransformationMatrix& b, bool transA,
                                          bool transB)
{
	...
	if (this->matrix != a.matrix && this->matrix != b.matrix && a.matrix->owner == 1 && b.matrix->owner == 1 && this
	                                                                                                            ->matrix
	                                                                                                            ->owner
		== 1)
...
Point3d TransformationMatrix::operator*(const Point3d& p) const
{
	...
	double w = (MATRIX_AT(m, 3, 0) * p.x + MATRIX_AT(m, 3, 1) * p.y + MATRIX_AT(m, 3, 2) * p.z + MATRIX_AT(m, 3, 3));
...
Vector3d TransformationMatrix::getLocalCoordTranslation() const
{
	...
	result.x = MATRIX_AT(m, 0, 0) * MATRIX_AT(m, 0, 3) + MATRIX_AT(m, 1, 0) * MATRIX_AT(m, 1, 3) + MATRIX_AT(m, 2, 0) *
		MATRIX_AT(m, 2, 3);
	result.y = MATRIX_AT(m, 0, 1) * MATRIX_AT(m, 0, 3) + MATRIX_AT(m, 1, 1) * MATRIX_AT(m, 1, 3) + MATRIX_AT(m, 2, 1) *
		MATRIX_AT(m, 2, 3);
	result.z = MATRIX_AT(m, 0, 2) * MATRIX_AT(m, 0, 3) + MATRIX_AT(m, 1, 2) * MATRIX_AT(m, 1, 3) + MATRIX_AT(m, 2, 2) *
		MATRIX_AT(m, 2, 3);
	return result;
}
...
*/
void TransformationMatrix::setToCoordFrameTransformation(const Vector3d& x, const Vector3d& y, const Vector3d& z,
...
void TransformationMatrix::setToInverseCoordFrameTransformation()
{
	...
	Vector3d r = this->getLocalCoordTranslation();
	//this is the negative of the displacement between the global frame and the local one, expressed in local
